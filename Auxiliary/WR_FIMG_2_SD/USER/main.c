...
#define base_sector  50000

u16 StartSector = base_sector;
u8 a[512];
u8 LCD_Out_Buf[512];
u8 SD_Out_Buf[512];
...
//!---------------------------------------
	...
	{	
		...
		{	
			if(StartSector>base_sector)
				StartSector -= 300;
			LCD_Clear(WHITE);
			set_window(0,0,320,240);
			LCD_ShowString(20,100,200,16,16,"Present Image:");
			LCD_ShowNum(200,100,((StartSector-base_sector)/300)+1,8,16);
			LCD_ShowString(20,130,200,16,16,"Present Sector:");
			LCD_ShowNum(200,130,StartSector,8,16);
			set_window(0,0,320,240);
		}
		
		else if(key == WKUP_PRES) //!按键WKUP图片索引++
		{	
			if(StartSector<60000)
				StartSector += 300;
			LCD_Clear(WHITE);
			set_window(0,0,320,240);
			LCD_ShowString(20,100,200,16,16,"Present Image:");
