...
{			   
	if(PEN==0)//有按键按下
	{
		...
		else if(TP_Read_XY2(&tp_dev.x,&tp_dev.y))//读取屏幕坐标
		{
			...
			if(flag_mir) tp_dev.x = 319 - tp_dev.x;
	 	} 
...
{	  
	POINT_COLOR=RED;
	// LCD_ShowString(20,140,16,"x1:",1);
 	// LCD_ShowString(20+70,140,16,"y1:",1);
 	// LCD_ShowString(20+140,140,16,"x2:",1);
 	// LCD_ShowString(20+210,140, 16,"y2:",1);
	// LCD_ShowString(20,160, 16,"x3:",1);
 	// LCD_ShowString(20+70,160, 16,"y3:",1);
	// LCD_ShowString(20+140,160,16,"x4:",1);
 	// LCD_ShowString(20+210,160, 16,"y4:",1);  
 	// LCD_ShowString(20,200, 16,"fac is:",1);     
	// LCD_ShowNum(20+24,140,x0,4,16);		//显示数值
	// LCD_ShowNum(20+24+70,140,y0,4,16);	//显示数值
	// LCD_ShowNum(20+24+140,140,x1,4,16);		//显示数值
	// LCD_ShowNum(20+24+210,140,y1,4,16);	//显示数值
	// LCD_ShowNum(20+24,160,x2,4,16);		//显示数值
	// LCD_ShowNum(20+24+70,160,y2,4,16);	//显示数值
	// LCD_ShowNum(20+24+140,160,x3,4,16);		//显示数值
	// LCD_ShowNum(20+24+210,160,y3,4,16);	//显示数值
 	// LCD_ShowNum(20+56,200,fac,3,16); 	//显示数值,该数值必须在95~105范围之内
	// Show_Str(20, 140, RED, BLACK, "准确率",16,0);
	// LCD_ShowString(68,140,16,":",1);
	// LCD_ShowNum(84,140,fac,3,16); 	//显示数值,该数值必须在95~105范围之内
	Show_Str(120, 140, RED, BLACK, "请重新校准",16,0);
}
...
{								 
	...
	LCD_Clear(BLACK);//清屏 	   
	POINT_COLOR=RED;
	Show_Str(25, 112, GREEN, BLACK, "请用校准笔依次点击红色光标进行校准",16,0);
		...
		{	
			...
			{			   
				case 1:						 
					...
					break;
				case 2:
 					TP_Drow_Touch_Point(lcddev.width-20,20,BLACK);	//清除点2
					TP_Drow_Touch_Point(20,lcddev.height-20,RED);	//画点3
					break;
				case 3:
 					TP_Drow_Touch_Point(20,lcddev.height-20,BLACK);			//清除点3
				...
				case 4:	 //全部四个点已经得到
					...
					{
						cnt=0;
 				    	TP_Drow_Touch_Point(lcddev.width-20,lcddev.height-20,BLACK);	//清除点4
   	 					TP_Drow_Touch_Point(20,20,RED);								//画点1
					...
					POINT_COLOR=BLUE;
					LCD_Clear(BLACK);//清屏
					// LCD_ShowString(50,112, 16,"Touch Screen Adjust OK!",1);//校正完成
					Show_Str(128, 112, GREEN, BLACK, "校准成功",16,0);
					delay_ms(200);
					TP_Save_Adjdata();
					ReturnHP();  
 					// LCD_Clear(WHITE);//清屏   
...
/*****************************************************************************
 * @name       :void TP_Prepare
 * @date       :2020-11-01
 * @function   :Preprare to use TP function.Set SPI IO functions as GPIO
 * @parameters :None
******************************************************************************/  
void TP_Prepare(void)
{
	GPIOA->CRL&=0XFF0FFFFF;GPIOA->CRL|=3<<20; //PA5 TCLK
	GPIOA->CRL&=0XF0FFFFFF;GPIOA->CRL|=8<<24; //PA6 MISO
	GPIOA->CRL&=0X0FFFFFFF;GPIOA->CRL|=3<<28; //PA7 MOSI
}
